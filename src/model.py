...
class RWKV(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        for n, p in self.named_parameters():
            if ("att.w0" in n) and (args.layerwise_lr > 0):
                lr_2x.add(n)
        ...
        if args.layerwise_lr > 0:
            optim_groups += [{"params": [param_dict[n] for n in lr_2x], "weight_decay": 0.0, "my_lr_scale": 2.0},]
...
"""
        ...
        for n in self.state_dict():
            ...
            scale = 1.0
            if "ln_" in n or ".ln" in n or "time_" in n or n.endswith('_w') or n.endswith('_w1') or n.endswith('_w2') or n.endswith('_bias') or (".weight" not in n):
                if 'ln_x.weight' in n:
                ...
                if ".att.output." in n or ".ffn.value." in n:
                    scale = 0
                    nn.init.zeros_(m[n])
                elif ".att.key." in n or ".att.gate." in n:
                    scale = 0.1
                    nn.init.orthogonal_(m[n], gain=0.1)
                else:
                    nn.init.orthogonal_(m[n], gain=1)
                print(f" [scale {scale}]")
